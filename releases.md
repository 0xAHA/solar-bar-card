...
### Changes
- **Total house consumption**: The Usage stats tile now shows total house consumption calculated from the energy balance (solar - export + import + battery discharge - battery charge). This physics-based formula works correctly regardless of whether `self_consumption_entity` reports solar self-consumption or total house load — fixing issue #53 where the Usage tile mirrored production when solar was less than actual consumption.
